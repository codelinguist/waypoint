...
import { fetchGoals, HouseholdNotFoundError } from '../api/client';
...
/**
 * Loads and refreshes a household's financial goals. Race safety and
 * refresh-failure retention match `useFinancialPosition` exactly — see that
 * hook's doc comment for the invariants this mirrors.
 */
export function useGoals(householdId: string) {
  ...
  const load = useCallback(
    async (mode: 'initial' | 'refresh') => {
      ...
      try {
        const data = await fetchGoals(householdId, controller.signal);
