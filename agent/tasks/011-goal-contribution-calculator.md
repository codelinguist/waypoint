...
Find the equal monthly saving amount needed to close an explicitly supplied monetary goal gap over an explicit number of contributions.
...
- Implement the READY [product brief](../product/goal-contribution-calculator/product-brief.md), including every acceptance criterion.
- Add pure domain calculation, typed validation, POST `/api/planning/goal-contribution-calculator`, dedicated tests and feature-local `api.md` examples.
...
- Exclusive code ownership: `backend/src/main/java/com/waypoint/planning/goalcontribution/**` and `backend/src/test/java/com/waypoint/planning/goalcontribution/**`.
- Exclusive prose/evidence ownership: `agent/product/goal-contribution-calculator/**`. Follow the brief's explicit shared-prose exception: defer central log, README and decision-log updates to a post-batch consolidation; do not edit shared files.
...
- Start implementation in a fresh conversation and isolated branch/worktree `task/011-goal-contribution-calculator`. The frontmatter slug and branch are deliberately blank per the user's queue-writing instruction; the filename and this body identify them unambiguously. Lifecycle fields belong to the orchestrator after queueing.
