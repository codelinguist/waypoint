...
- Narrow layout: `evidence/screenshots/direction-a-populated-narrow.png` (390px). Same structure; each record table lives in its own `overflow-x: auto` container so only the table scrolls horizontally, never the page (verified: `document.documentElement.scrollWidth === 390` at a 390px viewport for every evidence file).
