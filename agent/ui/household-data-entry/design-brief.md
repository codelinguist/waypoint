...
## Revision 1 (resolves independent review findings)

An independent fresh-subagent review of PR #57 (revision `2dcf047`) returned `RETURNED`, with one BLOCKING and one RECOMMENDED finding. Resolution:

- **BLOCKING — `FinancialGoal` had no provenance field at all.** Unlike Asset, Liability, IncomeStream, Obligation, FinancialSnapshot, and PlanningAssumption (all of which hardcode `SourceType.MANUAL_ENTRY` server-side), the backend `FinancialGoal` entity never had a `SourceType` column — the PR's "every persisted value carries a provenance tag" claim was true for 6 of 7 entities but false for goals. Fixed at the backend: `V9__add_financial_goal_source_type.sql` (additive `ALTER TABLE ... ADD COLUMN ... DEFAULT 'MANUAL_ENTRY'`, matching the same idiom `V7`/`V8` already used), a `sourceType` field on the `FinancialGoal` entity hardcoded to `MANUAL_ENTRY` in its constructor (identical pattern to every sibling entity), and `sourceType` added to `FinancialGoalResponse`. The frontend `FinancialGoal` type gained the field and `GoalsTable` gained a `Source` badge column, matching every other entity table's existing display convention (a minimal, mechanical consistency fix directly tied to the field the finding required — not a new design decision, so no fresh Product Owner approval was needed for it).
- **RECOMMENDED — liability-correction flow had no dedicated unit test, and the real-backend smoke spec never exercised it.** Added `CorrectLiabilityBalanceForm.test.tsx` (mirroring `CorrectAssetValuationForm.test.tsx`'s load-then-correct and 409-conflict/reload coverage), and extended `data-entry-smoke.spec.ts` to correct the liability's balance too, confirming the prior balance is still recoverable via the real `GET .../liabilities/{id}/balances` history endpoint.
- The OPTIONAL evidence-screenshot note was not separately actioned; the two screenshots already present were regenerated as part of re-running the smoke flow for this revision.

Automated checks after revision: `./verify.sh` — backend `BUILD SUCCESS`, `Tests run: 829, Failures: 0, Errors: 0, Skipped: 0` (net-zero: 2 new goal-provenance assertions were added, no other test count changed); frontend `Test Files 23 passed, Tests 117 passed`; `frontend/e2e/real-backend-data-entry-smoke.sh` `PASS`, now covering both the asset and liability correction flows against the real backend.
