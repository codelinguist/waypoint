...
class FutureValueApiIntegrationTest {
    ...
    @Test
    void growthUsesTheExactProductNotAPreRoundedMonthlyRate() throws Exception {
        // R1 regression: opening 6.00 * annual 1.00% / 1200 = 0.005 exactly, HALF_UP to 0.01.
        calculate(request("USD", "6.00", "0", "1.00", 1))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.schedule[0].growth").value(0.01))
                .andExpect(jsonPath("$.schedule[0].closingBalance").value(6.01))
                .andExpect(jsonPath("$.endingValue").value(6.01));
    }

    @Test
    void growthReconcilesForANonTerminatingRateOverALargeAcceptedBalance() throws Exception {
        // R1 regression: 7.00% annual is a non-terminating 7/1200; independently derived
        // growth 58333333333.33 over this balance, not the pre-rounded 58333333330.00.
        calculate(request("USD", "9999999999999.99", "250.00", "7.00", 1))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.schedule[0].growth").value(58333333333.33))
                .andExpect(jsonPath("$.schedule[0].closingBalance").value(10058333333583.32))
                .andExpect(jsonPath("$.endingValue").value(10058333333583.32))
                .andExpect(jsonPath("$.totalGrowth").value(58333333333.33));
    }
