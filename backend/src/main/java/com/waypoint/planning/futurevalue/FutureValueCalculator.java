 ...
 *   <li>Each month, growth is computed from the exact product of the opening balance and the
 *       annual percentage rate, divided by 1200, with a single HALF_UP rounding to 2 decimal
 *       places — never a pre-rounded monthly rate — before the equal monthly contribution is
 *       added at month end: {@code closing = opening + round(opening * annualRatePercentage /
 *       1200) + contribution}.
...
public class FutureValueCalculator {
    ...
    public FutureValueResult calculate(
        ...
        for (int month = 1; month <= projectionMonths; month++) {
            BigDecimal growth = opening.multiply(normalizedRate)
                    .divide(MONTHS_TIMES_HUNDRED, MONEY_SCALE, MONEY_ROUNDING);
